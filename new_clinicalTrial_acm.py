# 반복문
# 제목-> nct_id에 있는지 search하고 없으면 -> data_extract_Biolinkbert(정보추출 파일) 돌리고 -> db에 넣어두고
# 있으면 끝
import requests
...
#RSS feed 받아오기
f = open('./NCT_ID.txt','w')
url = 'https://clinicaltrials.gov/ct2/results/rss.xml?rcv_d=14&lup_d=&sel_rss=new14&term=Drug&type=Intr&intr=Drug&count=10000'
nct_ls = []
...
if response.status_code == 200:
    ...
    link_ls = soup.select('item > guid')
else : 
    print(response.status_code)

for i in range(len(link_ls)):
  nct_id = link_ls[i].get_text()
  nct_ls.append(nct_id)
  f.write(nct_id)

# DB에 업로드
# Making Connection
myclient = MongoClient("mongodb://test1:test1234@3.35.243.113:27017/?authMechanism=DEFAULT&authSource=admin")
...
#os.system('resource_control_acm.py')
...
# rss로 받은 nct 별로 data_extract_Biolinkbert(정보추출 파일) 돌리기
for i in range(len(nct_ls)):
    query = {"_id": nct_ls[i]}
    url = "https://www.clinicaltrials.gov/api/query/full_studies?expr="+nct_ls[i]+"&min_rnk=1&max_rnk=&fmt=json"
    response = requests.get(url).json()
    update_date = response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['StatusModule']['LastUpdatePostDateStruct']['LastUpdatePostDate']
    print("now: "+nct_ls[i])
    # DB에 이미 있으면
    if(Collection.count_documents(query) == 1):
        print("inside DB")
        # 해당 파일 update 날짜 비교
        if(Collection.find_one({"update_date": update_date}) is None):
            print("Date is different")
            # 다르면 삭제하고 다시 돌리고 DB에 넣기
            Collection.find_one_and_delete(query)
            url = "https://www.clinicaltrials.gov/api/query/full_studies?expr="+nct_ls[i]+"&min_rnk=1&max_rnk=&fmt=json"
            try:
                if(response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['DesignModule']['DesignInfo']['DesignInterventionModel']=="Parallel Assignment" or "Single Group Assignment" or "Crossover Assignment"):
                    for k in range(len(response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['ArmsInterventionsModule']['InterventionList']['Intervention'])):
                        if response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['ArmsInterventionsModule']['InterventionList']['Intervention'][k]['InterventionType'] == "Drug":
                            try:
                                request_call(url)
                                for filename in os.listdir("./NCT_ID_database_acm"):
                                    if(nct_ls[i]+'.json' == filename):
                                        print("db..uploading"+filename)
                                        with open(os.path.join("./NCT_ID_database_acm", filename), 'r') as file:
                                ...
                                break
                            except (RuntimeError, TypeError, NameError, UnicodeEncodeError, UnboundLocalError, KeyError, UnboundLocalError):
                                continue
                            except (json.decoder.JSONDecodeError,requests.exceptions.JSONDecodeError):
                                continue
            except (KeyError, UnboundLocalError, requests.exceptions.JSONDecodeError):
                continue
        else:
            print("Date is same")
            # 같으면 pass
            pass
    # DB에 없으면 바로 돌리고 DB에 넣기
    else:
        print("no file in DB")
        try:
            if(response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['DesignModule']['DesignInfo']['DesignInterventionModel']=="Parallel Assignment" or "Single Group Assignment" or "Crossover Assignment"):
                for k in range(len(response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['ArmsInterventionsModule']['InterventionList']['Intervention'])):
                    if response['FullStudiesResponse']['FullStudies'][0]['Study']['ProtocolSection']['ArmsInterventionsModule']['InterventionList']['Intervention'][k]['InterventionType'] == "Drug":
                        print("Intervention Model & Intervention Type is fit")
                        try:
                            request_call(url)
                            for filename in os.listdir("./NCT_ID_database_acm"):
                                if(nct_ls[i]+'.json' == filename):
                        ...
                        print("InterventionType is NOT fit")
                        
            else:
                print("InterventionModel is NOT fit")
        except (KeyError, UnboundLocalError, requests.exceptions.JSONDecodeError):
            continue
